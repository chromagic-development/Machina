/*
	...
	4/1/24 Modified to trigger virtual Alt-Z key combination
*/
...
namespace PorcupineMachina
{
    /// <summary>
    /// Microphone Machina for Porcupine wake word engine. It creates an input audio stream from a microphone, monitors it, and
    ...
    public class Machina
    {
            ...
            using (Porcupine porcupine = Porcupine.FromKeywordPaths(accessKey, keywordPaths, modelPath, sensitivities))
            {
                ...
                using (PvRecorder recorder = PvRecorder.Create(frameLength: porcupine.FrameLength, deviceIndex: audioDeviceIndex))
                {
                    ...
                    while (recorder.IsRecording)
                    {
                        ...
                        if (result >= 0)
                        {
                            // Console.WriteLine($"[{DateTime.Now.ToLongTimeString()}] Detected '{keywordNames[result]}' spoken to trigger virtual Alt-Z key combination");
        ...
        public static void Main(string[] args)
        {
            ...
            // run Machina with validated arguments
            RunMachina(accessKey, modelPath, keywordPaths, sensitivities, audioDeviceIndex, outputPath);
        }
